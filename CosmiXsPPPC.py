...
def get_main_readplot(speciesFlux_folder, prop_scheme, prop_set, DM_profile, mdm0, channel): 
    ...
    return K_arr_out_IS, gaussian_filter1d(species_ch_interp, sigma_Gauss), Kout_TOA, gaussian_filter1d(species_ch_TOA, sigma_Gauss)      ## GeV vs. GeV^-1.m^-2.s^-1.sr^-1
#########################################################################################################################
    ...
    for j in range(0, len(DM_profile_list)):
        ...
        for mi in range(0, len(mdm0_arr)):
            ...
            for ci in range(0, len(channel_list)):
                ...
                ##----------------------------------------------------------------------------------------
                ax1.plot(K_out_TOA[0:1+np.argmin(species_ch_out_MAX_TOA)]/n_val, (K_out_TOA**2*species_ch_out_MAX_TOA)[0:1+np.argmin(species_ch_out_MAX_TOA)], lw=2.6, dashes=[4,2], color='b', alpha=1, label="MAX") 
                
                ax1.plot(K_out_TOA[0:1+np.argmin(species_ch_out_MED_TOA)]/n_val, (K_out_TOA**2*species_ch_out_MED_TOA)[0:1+np.argmin(species_ch_out_MED_TOA)], lw=2.6, ls='-', color='b', alpha=1, label="MED") 
                
                ax1.plot(K_out_TOA[0:1+np.argmin(species_ch_out_MIN_TOA)]/n_val, (K_out_TOA**2*species_ch_out_MIN_TOA)[0:1+np.argmin(species_ch_out_MIN_TOA)], lw=2.8, dashes=[1,2], color='b', alpha=1, label="MIN") 
                
                plt.fill_between(K_out_TOA[0:1+np.argmin(species_ch_out_MAX_TOA)]/n_val, (K_out_TOA**2*species_ch_out_MIN_TOA)[0:1+np.argmin(species_ch_out_MAX_TOA)], (K_out_TOA**2*species_ch_out_MAX_TOA)[0:1+np.argmin(species_ch_out_MAX_TOA)], facecolor='b', edgecolor='None', alpha=0.4)
                ##----------------------------------------------------------------------------------------
